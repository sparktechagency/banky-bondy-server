...
export const getMatchingBondRequest = async (
    ...
    // 8. DFS for cycles (unique user constraint added)
    const seenCycles = new Set<string>();
    const dfs = (
        startId: string,
        currentId: string,
        path: string[],
        accScore: number,
        userSet: Set<string> // FIX: track users in the path
    ) => {
        if (path.length > maxCycleSize) return;

        const currentReq = requestMap.get(currentId);
        ...
        for (const { to, score: nextScore } of edges.get(currentId) || []) {
            ...
            // FIX: skip if this user already exists in chain
            if (userSet.has(nextReq.user.toString())) continue;
            ...
            newUserSet.add(nextReq.user.toString());

            dfs(startId, to, [...path, to], newScore, newUserSet);
        }
    };

    // Start DFS with initial user set containing start user
    dfs(bondRequestId, bondRequestId, [bondRequestId], 1, new Set([userId]));
