...
// Helper Functions for "Surprise", "Entry", "Empty"
const isEmpty = (s?: string) => normalizeText(s) === 'empty';
const isSurprise = (s?: string) => normalizeText(s) === 'surprise';
const isEntry = (s?: string) => normalizeText(s) === 'entry';
...
const isValidMatch = (want: string, offer: string): boolean => {
    ...
    // Surprise matches everything
    if (isSurprise(normalizedWant) || isSurprise(normalizedOffer)) return true;

    // Entry matches Entry with cosine similarity
    if (isEntry(normalizedWant) && isEntry(normalizedOffer)) return true;

    // Empty can only match with Entry
    if (
        (isEmpty(normalizedWant) && isEntry(normalizedOffer)) ||
        (isEmpty(normalizedOffer) && isEntry(normalizedWant))
    )
        return true;

    return false; // Default if none of the conditions match
};
...
export const getMatchingBondRequest = async (
    ...
    // Fetch the starting bond request
    const startRequest = await BondRequest.findOne({
        _id: bondRequestId,
        user: userId,
        status: ENUM_BOND_REQUEST_STATUS.WAITING_FOR_LINK,
    ...
    // Ensure embeddings exist for start request
    if (!startRequest.offerVector?.length) {
        startRequest.offerVector = await generateEmbedding(startRequest.offer);
        await BondRequest.updateOne(
            { _id: startRequest._id },
    ...
    // Ensure embeddings for candidates (once)
    for (const candidate of candidates) {
        if (!candidate.offerVector?.length) {
            candidate.offerVector = await generateEmbedding(
                candidate.offer || ''
    ...
    const getMatchType = (want: string, offer: string): string => {
        const normalizedWant = normalizeText(want);
        const normalizedOffer = normalizeText(offer);

        // "Surprise" matches everything, return "Perfect Match"
        if (isSurprise(normalizedWant) || isSurprise(normalizedOffer))
            return 'Perfect Match';

        // "Entry" to "Entry" matches with cosine similarity, return "Cosine"
        if (isEntry(normalizedWant) && isEntry(normalizedOffer))
            return 'Cosine';

        // "Empty" can only match with "Entry", return "Perfect Match" if matched
        if (
            (isEmpty(normalizedWant) && isEntry(normalizedOffer)) ||
            (isEmpty(normalizedOffer) && isEntry(normalizedWant))
        )
    ...
    // Pairwise matches
    for (const candidate of candidates) {
        // Check match type based on Surprise, Entry, Empty logic
        const matchType1 = getMatchType(startRequest.want, candidate.offer);
        const matchType2 = getMatchType(candidate.want, startRequest.offer);

        if (matchType1 === 'No Match' || matchType2 === 'No Match') continue;

        // Calculate scores based on match types
        let score1 = 0;
        let score2 = 0;

        if (matchType1 === 'Perfect Match') {
            score1 = 1; // Perfect match gives the highest score
        } else if (matchType1 === 'Cosine') {
            score1 = cosineSimilarity(
                startRequest.wantVector,
                candidate.offerVector
            ); // Cosine similarity
        }

        if (matchType2 === 'Perfect Match') {
            score2 = 1;
        } else if (matchType2 === 'Cosine') {
            score2 = cosineSimilarity(
                candidate.wantVector,
                startRequest.offerVector
            );
        }

        // Ensure score meets the minimum threshold
        if (score1 < MIN_SCORE || score2 < MIN_SCORE) continue;
    ...
    // Handle DFS for cycles (remains as-is)
    const requestMap = new Map<string, any>(
        candidates.map((c) => [c._id.toString(), c])
    );
    requestMap.set(bondRequestId, startRequest);
    ...
    // DFS to find cycles (3-5 user chains)
    const dfs = (
        startId: string,
        currentId: string,
        path: string[],
    ...
    const startIndex = (page - 1) * effectiveLimit;
    const paginated = matches.slice(startIndex, startIndex + effectiveLimit);
